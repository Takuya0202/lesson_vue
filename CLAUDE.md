...
| # | 課題 | 使用技術 | 前提 | ステータス |
|---|------|---------|------|---------|
| 3-1 | タスク管理アプリ | `ref`, `computed`, `v-for`, `v-if`, `props`, `emits`, コンポーネント分割 | Phase 1 | ⬜ 未着手 |
| 3-2 | ユーザー管理ダッシュボード | Vue Router, Pinia, `onMounted`, `watch`, `computed`, `provide/inject` | Phase 1 | ⬜ 未着手 |
| 3-3 | ショッピングカート | Pinia（複数ストア）, Composables, `computed`, slots, Router | 1-10 まで | ⬜ 未着手 |
| 3-4 | UI ライブラリで管理画面 | shadcn-vue or Vuetify, Router, Pinia, Composables | Phase 1.5 | ⬜ 未着手 |

#### 3-1 タスク管理アプリ
- タスクの追加・削除・完了トグルができる
- 「すべて / 未完了 / 完了済み」のフィルター切り替え
- 残タスク数をリアルタイム表示
- タスク入力欄が空のときは追加ボタンを非活性にする
- コンポーネントは最低3つに分割する（入力フォーム、タスクリスト、タスク1行）

#### 3-2 ユーザー管理ダッシュボード
- `/users` でユーザー一覧を表示（データは Pinia ストアで管理）
- `/users/:id` でユーザー詳細ページに遷移
- 一覧ページに検索フィルター（名前で絞り込み）
- ユーザーの追加・削除機能
- ナビゲーションバーにユーザー総数をリアルタイム表示（provide/inject or Pinia）
- ページ遷移時にローディング表示を入れる

#### 3-3 ショッピングカート
- 商品一覧ページ（`/products`）とカートページ（`/cart`）を作る
- 商品ストア（商品データ）とカートストア（カート内容）を分離
- 「カートに追加」ボタンで数量が増える。同じ商品は数量を加算
- カートページで数量変更・商品削除ができる
- 合計金額・合計点数を `computed` でリアルタイム計算
- 価格のフォーマット処理を Composable（`useFormatPrice`）に切り出す
- ヘッダーにカート内の点数をバッジ表示（全ページ共通）

#### 3-4 UI ライブラリで管理画面
- shadcn-vue or Vuetify の Table / Dialog / Form / Button を使って管理画面を構築
- データの CRUD（一覧表示・新規作成ダイアログ・編集・削除確認ダイアログ）
- フォームにバリデーションを入れる（必須チェック、文字数上限など）
- ダークモード切り替えトグルを実装
- レスポンシブ対応（モバイルでも崩れない）
